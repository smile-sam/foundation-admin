  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        ...
        sortable="custom"
        :class-name="getSortClass('account')"
      >
        <template slot-scope="scope">
...
import { page, addUser, updateUser, deleteUser } from '@/api/user/user'
...
export default {
  ...
  methods: {
    ...
    handleUpdate ({ $index, row }) {
      ...
      this.imageUrl = 'http://localhost:9527/user/file/image/' + this.temp.avatar + '?authToken=' + getToken()
      this.temp.sex = String(this.temp.sex)
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.temp.avatar = res // URL.createObjectURL(file.raw)
      // this.$set(this.temp, 'avatar', 'https://www.baidu.com/img/flexible/logo/pc/index.png')
      console.log(this.temp.avatar)
    },
    beforeAvatarUpload(file) {
      console.log(file.type)
      const pictureFormats = ['JPG', 'PNG', 'JPEG', 'GIF', 'image/jpeg', 'image/png', 'jpg', 'png', 'jpeg', 'gif']
      // const isJPG = file.type === 'image/jpeg'
      const isLt2M = file.size / 1024 / 1024 < 2
      const isValid = pictureFormats.includes(file.type)
      if (!isValid) {
